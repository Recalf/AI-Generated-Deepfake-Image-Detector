...
The model detects images generated by many state-of-the-art (SOTA) generative models, including **DALL-E3, Flux, Nano Banana Pro, Diffusion Models (SDXL, SD3.5...), Midjourney V6** and more, classifying them as fake.
...
**Side Note:** The LLRD/Cosine recipe was originally designed for fine-tuning ViTs (MAE and BEiT research papers), but it works well on ConvNeXt even though it's a CNN architecture.<br>Shortly, it’s because ConvNeXt was designed to act like ViTs while keeping CNN architecture efficiency.
# Project Structure
├── train/<br>
...
- Build Image:
`docker image build -t ai-image-detector .` make sure you're in the repo root (where the Dockerfile is)
- Run Container :
