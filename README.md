...
(Side note: The model is a CNN architecture (light/fast) and is trained on ~400k images (a medium-sized dataset). Don’t expect it to be SOTA level, also its not trained on partially AI generated images)
# Test Score (OOD):
EvalGen (SOTA generative models): **90.40% fake detection rate**
